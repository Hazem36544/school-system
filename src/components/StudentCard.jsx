...
const StudentCard = ({ student }) => {
  // Added "Registered" status as default because server doesn't currently send student rating
  ...
  // Prepare real variables from API (based on passing from Dashboard/Search)
  const displayName = student.name || 'طالب غير محدد';
  const displayAge = student.age ? `${student.age} سنة` : 'العمر غير متوفر';
  const displayGender = student.gender === 'Male' ? 'ذكر' : (student.gender === 'Female' ? 'أنثى' : 'غير محدد');
  const displayStatus = student.status === 'مسجل' || !student.status ? 'مسجل' : student.status;

  return (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm mb-6 border border-gray-100/50 hover:shadow-xl transition-all duration-300" dir="rtl">
      {/* Header Section */}
      <div className="flex justify-between items-start mb-8">
        ...
        <div className={`px-4 py-1.5 rounded-full text-sm font-bold ${getStatusColor(displayStatus)}`}>
          {displayStatus === 'Excellent' ? 'ممتاز' :
            displayStatus === 'Very Good' ? 'جيد جداً' :
              displayStatus === 'Good' ? 'جيد' :
                displayStatus === 'Registered' ? 'مسجل' : displayStatus}
        </div>
      </div>
      ...
      <div className="flex gap-4 mb-8">
        ...
        <div className="flex-1 bg-gray-50 rounded-[1.5rem] p-4 flex flex-col items-center justify-center gap-2 border border-gray-100">
          ...
          <div className="text-center">
            <span className="block text-lg font-extrabold text-gray-800 text-xs text-gray-500">غير متوفر</span>
