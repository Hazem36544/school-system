...
const SchoolAccount = () => {
    ...
    // جلب بيانات المدرسة من السيرفر
    useEffect(() => {
        const fetchSchoolData = async () => {
            try {
                setLoading(true);
                const response = await schoolAPI.getCurrentSchool();
                // التأكد من أن البيانات موجودة في response.data
                if (response.data) {
    ...
    // ربط المتغيرات بدقة مع حقول السواجر (SchoolResponse)
    const schoolName = schoolData?.name || 'مدرسة غير محددة';
    const schoolEmail = schoolData?.email || 'غير متوفر';
    const schoolPhone = schoolData?.contactNumber || 'غير متوفر';
    const schoolCode = schoolData?.username || (schoolData?.id ? schoolData.id.substring(0, 8).toUpperCase() : 'غير متوفر');
            ...
            <div className="flex-1 mr-20 p-8 min-h-screen overflow-y-auto">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        {/* بطاقة المدرسة (على اليمين) - 4 Cols */}
                        <div className="lg:col-span-4 space-y-6 order-1">
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                                <div className="flex items-center gap-3 mb-3 text-gray-800 font-bold text-sm">
                        ...
                        {/* المعلومات الأساسية (على اليسار) - 8 Cols */}
                        <div className="lg:col-span-8 order-2">
                            <div className="bg-white p-8 rounded-3xl shadow-sm h-full border border-gray-100">
